...
import 'package:checkers_mobile_client/widgets/animated_snackbar.dart';
...
class SnackbarService with ChangeNotifier {
  ...
  void displaySnackbar(String message) {
    final time = DateTime.now().toIso8601String().hashCode ^
        message.hashCode ^
        _snackbars.length;
    final snackbar = AnimatedSnackbar(
        message: message + snackbars.length.toString(),
  ...
  void _removeSnackbar(String timestamp) {
    if (_snackbars.where((element) => element.timeStamp == timestamp).isEmpty)
      return;
